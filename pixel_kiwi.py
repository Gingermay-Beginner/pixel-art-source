...
# 姜饼人胳膊延伸（x=20~22, y=20~21）
for _ay in range(20, 22):
    for _ax in range(20, 23):
        canvas[_ay][_ax] = GB
# 复制胳膊（y=27~28）
for _ay in range(27, 29):
    for _ax in range(20, 23):
        canvas[_ay][_ax] = GB
# 兔子胳膊（y=6~7 和 y=13~14）
for _ay in range(6, 8):
    for _ax in range(20, 23):
        canvas[_ay][_ax] = BUN
for _ay in range(13, 15):
    for _ax in range(20, 23):
        canvas[_ay][_ax] = BUN
# 清除旧位置孤儿像素
for _ax in range(20, 23):
...
# ── 左右两侧半张桌子（各露出4格）──
# 左桌：x=0~8（加宽1格），含木纹和桌腿
fill(canvas, TY1, TY2, 0, 8, TABLE)
for x in range(2, 9, 5):
    for y in range(TY1+1, TY2): set_px(canvas, x, y, TABLE_D)
for y in range(TY1, TY2+1): set_px(canvas, 8, y, TABLE_EDGE)
wrow(canvas, TY2, 0, 8, TABLE_EDGE)
if TY2+1 < H: set_px(canvas, 8, TY2+1, TABLE_LEG)
# 右桌：x=59~63（左移1格），含木纹和桌腿
fill(canvas, TY1, TY2, 59, W-1, TABLE)
for x in range(61, W-1, 5):
    for y in range(TY1+1, TY2): set_px(canvas, x, y, TABLE_D)
for y in range(TY1, TY2+1): set_px(canvas, 59, y, TABLE_EDGE)
wrow(canvas, TY2, 59, W-1, TABLE_EDGE)
if TY2+1 < H: set_px(canvas, 59, TY2+1, TABLE_LEG)
...
def draw_bun(buf, w, h):
    # 耳朵（短粗版，y=2~4，外侧加1格，顶圆角）
    fl(buf,2,4,3,4,BUN); fl(buf,2,4,6,7,BUN)
    sp(buf,3,2,(0,0,0,0)); sp(buf,7,2,(0,0,0,0))  # 外侧顶圆角
    fl(buf,3,4,4,4,BUNK); fl(buf,3,4,6,6,BUNK)
    # 头（y=5~10，x=2~8）
    fl(buf,5,10,2,8,BUN)
    ...
    # 手臂
    fl(buf,12,14,0,1,BUN)
    fl(buf,12,14,9,10,BUN)
    # 手臂顶角圆角
...
chair_img_opp = make_char_img(draw_chairback_opp, CBW, CBH-1)
...
# (53~56,14) 补椅背暗色
for gx in [53, 54, 55, 56]:
    for dy in range(S_):
        for dx in range(S_):
            _rgba_px[gx*S_+dx, 14*S_+dy] = (158, 130, 35, 255)
CREAM = (242, 235, 220, 255)
...
# 清除兔子旧胳膊格 y=5 和 y=15（x=20~22），露出桌面色
_table_row = [(188,145,72,255),(188,145,72,255),(172,130,60,255)]
for i, gx in enumerate(range(20, 23)):
    for gy in [5, 15]:
        for dy in range(S_):
            for dx in range(S_):
                _rgba_px[gx*S_+dx, gy*S_+dy] = _table_row[i]
# 清除姜饼人旧胳膊格 y=19 和 y=29（x=20~22），露出桌面色
for i, gx in enumerate(range(20, 23)):
    for gy in [19, 29]:
        for dy in range(S_):
            for dx in range(S_):
                _rgba_px[gx*S_+dx, gy*S_+dy] = _table_row[i]
# x=20 非胳膊身体格清为桌面色（胳膊保留：兔y=6~7,13~14；GB y=20~21,27~28）
_tx20 = (188, 145, 72, 255)
for gy in list(range(8, 13)) + list(range(22, 27)):
    for dy in range(S_):
        for dx in range(S_):
            _rgba_px[20*S_+dx, gy*S_+dy] = _tx20
# 清除垫子下方多余桌边线 y=17，x=45~53（保留x=44桌边）
for gx in range(45, 54):
    for dy in range(S_):
        for dx in range(S_):
            _rgba_px[gx*S_+dx, 17*S_+dy] = (205, 198, 188, 255)
...
draw_bag_fpx_rot(-1, 7, BAG_MG, BAG_MG_D)   # 军绿（兔子和姜饼人之间）
draw_bag_fpx_rot(-1, 21, BAG_MT, BAG_MT_D)   # 薄荷绿（姜饼人侧，靠下）
